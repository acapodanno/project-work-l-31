...
| L15 | Media | **Ruolo Supporto: «Prenota Visita» non utilizzabile.** La voce di menu e la rotta `/booking` sono consentite al Supporto, ma `bookAppointment` prenota sempre per il profilo dell'utente collegato e il Supporto non ha un profilo paziente: l'interfaccia mostra «Devi essere registrato come paziente per prenotare». L'API invece consente a Supporto e Medico di creare visite per qualunque paziente (`POST /api/appointments`). Il Medico ha un form dedicato («Nuovo appuntamento», con scelta del paziente) | `app-state.service.ts`, `booking-page.component.ts`, `app.component.html` | Aggiungere al form di prenotazione un selettore del paziente per il Supporto (la funzione `bookAppointmentForPatient` esiste già ma non è usata) oppure nascondere la voce al Supporto |
